...
def get_api_key_env(api_name: str) -> str:
    # get_api_key_env('google_api_key')
    ret_value = os.getenv(api_name)
    if ret_value is None:
        raise EnvironmentError(f"Environment variable '{api_name}' not found.")
...
# --- Gemini Configs ---
gemini_generate_config = {"temperature": 0.5}
...
class LS_GeminiNode:
    ...
    def run_gemini(self, model, system_prompt, user_prompt, max_output_tokens, temperature,
        ...
        genai.configure(api_key=get_api_key_env('google_api_key'), transport='rest')
        prompt = {
...
class LS_GeminiNode_V2:
    ...
    def run_gemini_v2(self, model, system_prompt, user_prompt, max_output_tokens, temperature,
        ...
        import google.generativeai as genai
        ret_texts = []
        genai.configure(api_key=get_api_key_env('google_api_key'))
        contents = []
        ...
        if image_1 is not None and image_2 is not None:
            for index,img in enumerate(image_1):
                images = [tensor2pil(img.unsqueeze(0)).convert('RGB')]
                _image2 = tensor2pil(image_2[index].unsqueeze(0)).convert('RGB') if index < len(image_2) else tensor2pil(image_2[-1].unsqueeze(0)).convert('RGB')
                ...
                response = genai.GenerativeModel(model).generate_content(
                    contents=contents,
                    generation_config={"temperature": temperature, "max_output_tokens": max_output_tokens, "seed": seed},
                    safety_settings=gemini_safety_settings
                )
                ret_text = response.text
                log(f"{self.NODE_NAME}: Gemini response is:\n\033[1;36m{ret_text}\033[m")
        ...
        elif image_1 is not None or image_2 is not None:
            _img = tensor2pil((image_1 or image_2)[0].unsqueeze(0)).convert('RGB')
            img_byte_arr = io.BytesIO()
            _img.save(img_byte_arr, format='PNG')
            img_byte_arr.seek(0)
            image_bytes = img_byte_arr.read()
            contents.append({
                "inline_data": {"mime_type": "image/png", "data": image_bytes}
...
class LS_OBJECT_DETECTOR_Gemini_V2:
    ...
    def run_gemini_detect_v2(self, image, model, prompt):
        ...
        log(f" Запуск run_gemini_detect_v2 {model}")
        ...
        for img in image:
            ...
            log(f" start response = genai.GenerativeModel(model).generate_content")
            response = genai.GenerativeModel(model).generate_content(
                contents=contents,
                safety_settings=gemini_safety_settings
            )
...
class LS_Gemini_Image_Edit:
    ...
    def run_gemini_image_edit(self, image, model, temperature, seed, user_prompt,
        ...
        for idx, img in enumerate(image):
            input_images = [tensor2pil(img.unsqueeze(0)).convert('RGB')]
            ...
            if len(input_images) > 1:
                contents[0]["text"] += "\nBased on the first image, use other reference images as guidance."
            else:
                contents[0]["text"] += "\nBased on this image."

            response = genai.GenerativeModel(model).generate_content(
                contents=contents,
                generation_config={"temperature": temperature, "seed": seed},
                safety_settings=gemini_safety_settings
            )

            for item in response.parts:
                if hasattr(item, "inline_data") and item.mime_type == "image/png":
