...
Как это работает:
    ...
    - После формирования файлов запускается aria2c:
        * для hf.txt:
...
def format_block_for_aria2(b: Block) -> str:
    # порядок вывода: URL, затем out, затем dir
...
def run_aria2_for_hf(new_model_file: str) -> int:
    """Запуск aria2c для HuggingFace (с auth header)."""
    try:
    ...
    for out in proc.stdout:  # type: ignore[union-attr]
...
def run_aria2_for_civitai(new_model_file: str) -> int:
    """Запуск aria2c для Civitai (без auth header, токен уже в URL)."""
    cmd = [
        "aria2c",
        f"--input-file={new_model_file}",
        "--allow-overwrite=false",
